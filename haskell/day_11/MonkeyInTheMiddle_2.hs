...
updateOp :: Monkey -> Int -> Monkey
updateOp monkey bigMod = monkey {op = (`mod` bigMod) . (op monkey)}
...
main = do
  ...
  let bigMod = foldl (\acc x -> acc * (testValue x)) 1 monkeyList
  let updatedMonkeyList = map (`updateOp` bigMod) monkeyList

  let initialItemCount = getItemCount updatedMonkeyList

  let finalItemCount = iterateN 10000 updatedMonkeyList [0, 0, 0, 0, 0, 0, 0, 0]
  let twoLargest = (take 2 . reverse . sort) finalItemCount

  print $ product twoLargest

  print finalItemCount
