...
extractList :: [Maybe Int] -> [Int]
extractList [] = []
extractList (x : xs) = case x of
  Just x -> x : extractList xs
  Nothing -> extractList xs
...
main = do
  ...
  let src = lookupKey 1 positions
  let dest = head $ lookupKey 27 positions

  let dist_list = map (\x -> dist (spots, positions) x dest) src

  print $ minimum (extractList dist_list)